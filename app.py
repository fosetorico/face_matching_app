...
# Face Detection =================================================
# MTCNN (Multi-Task Cascaded Convolutional Networks) is a deep learning model designed 
# for face detection and facial landmark localization
detector = MTCNN()
# VGGFace is a deep learning model specifically designed for face recognition and verification
model = VGGFace(model=model_name,include_top=include_tops,input_shape=(224,224,3),pooling=poolings)
feature_list = pickle.load(open(features_name,'rb'))  # rb = read binary mode
...
def save_uploaded_image(uploaded_image):
    try:
        ...
        with open(os.path.join(uploadn_path,uploaded_image.name),'wb') as f:  # wb = write binary mode
            f.write(uploaded_image.getbuffer())   # getbuffer() = save image
        return True
    except:
        return False
...
def recommend(feature_list, features):
    ...
    for i in range(len(feature_list)):
        similarity.append(cosine_similarity(features.reshape(1, -1), feature_list[i].reshape(1, -1))[0][0]) # reshape(1, -1) = 2d array
